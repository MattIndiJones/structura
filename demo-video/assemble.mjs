...
const marks = JSON.parse(fs.readFileSync(path.join(B, 'marks.json'), 'utf8'));
...
for (const s of PLAN) {
  ...
  const inputs = ['-ss', start.toFixed(3), '-t', dur.toFixed(3), '-i', path.join(RAW, 'app.webm')];
...
const FINAL = path.join(HERE, 'structura-ai-demo.mp4');
