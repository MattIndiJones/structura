...
- `PARAM() X` sans valeur → accepté, **en % par défaut**. À supprimer (D4).
  ...
  - Elle est présentée comme facultative dans `docs/reference/PAYSCRIPT_REFERENCE.md` (tableau des
    déclarations), `docs/projects/pricing/PAYSCRIPT_PARAM_PAR_OBSERVATION.md` et le mémo de l'éditeur
    (`PayScriptEditor.vue`).
...
4. **`PARAM()` à valeur obligatoire (D4).**
   ...
   - Reprendre `test_parser.py`, `docs/reference/PAYSCRIPT_REFERENCE.md`,
     `docs/projects/pricing/PAYSCRIPT_PARAM_PAR_OBSERVATION.md`, le mémo de l'éditeur et ce que l'assistant IA
     sait du langage.
