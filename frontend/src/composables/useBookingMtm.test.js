...
const saved = (changes = {}) => ({
  ...
  diagnostics: { request: { recalibrate: 'realized' } },
...
describe('MtM quotidien du Booking', () => {
  ...
  it('ouvre sur le marché actuel et aujourd’hui, même si le dernier run est ancien', async () => {
    ...
    expect(state.mtmModeFor(1)).toBe('realized')
    expect(state.mtmDateFor(1)).toBe('2026-09-17')
    expect(state.mtmResults[1]).toBeUndefined()
  })

  it.each([
    ['booking', () => ({ diagnostics: { request: { recalibrate: 'none' } } })],
    ...
    ['surcharge', () => ({ diagnostics: { request: { recalibrate: 'realized', r: 4 } } })],
    ['autre fenêtre', () => ({ diagnostics: { request: { recalibrate: 'realized', window_days: 60 } } })],
  ...
  it('restaure uniquement le résultat du jour et filtre les limites locales en UTC', async () => {
    ...
    expect(url.searchParams.get('recalibrate')).toBe('realized')
  })
